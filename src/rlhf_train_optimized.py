"""
Optimized RLHF training script with enhanced acceleration support.
"""
...
def log_performance_metrics(wandb_run, epoch, batch_time, gpu_stats, system_stats):
    ...
    if wandb_run:
        ...
        if gpu_stats:
            metrics.update({
                'performance/gpu_memory_used_gb': gpu_stats['memory_used'],
                'performance/gpu_memory_total_gb': gpu_stats['memory_total'],
                'performance/gpu_memory_percent': gpu_stats['memory_percent'],
                'performance/gpu_load_percent': gpu_stats['gpu_load']
            })
        
        if system_stats:
            metrics.update({
                'performance/system_memory_used_gb': system_stats['memory_used_gb'],
                'performance/system_memory_total_gb': system_stats['memory_total_gb'],
                'performance/system_memory_percent': system_stats['memory_percent']
            })
...
def train_rlhf_optimized(cfg: DictConfig) -> None:
    """Optimized main training function with enhanced acceleration."""
    
    # Add current timestamp
    cfg.now = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    
    # Print configuration
    print(f"Optimized RLHF Configuration:\n{OmegaConf.to_yaml(cfg)}")
    ...
    print(f"Loading model {cfg.model.name} with optimized settings...")
    
    # Enable flash attention if available and configured
    model_kwargs = {}
    if hasattr(cfg, 'acceleration') and cfg.acceleration.get('use_flash_attention', False):
        try:
    ...
    model = AutoModelForCausalLM.from_pretrained(
        cfg.model.name,
        torch_dtype=torch.float16 if hasattr(cfg, 'acceleration') and cfg.acceleration.get('mixed_precision') == 'fp16' else torch.float32,
        **model_kwargs
    )
    ...
    for epoch, batch in tqdm(enumerate(ppo_trainer.dataloader), total=cfg.training.num_train_epochs):
        ...
        system_stats = get_system_memory_usage()
        ...
        reward_stats['batch_time'].append(batch_time)
        
        if gpu_stats:
            reward_stats['gpu_memory_percent'].append(gpu_stats['memory_percent'])
            reward_stats['gpu_load_percent'].append(gpu_stats['gpu_load'])
        ...
        if epoch % 5 == 0:
            ...
            if gpu_stats:
                print(f"  GPU Memory: {gpu_stats['memory_used']}GB/{gpu_stats['memory_total']}GB ({gpu_stats['memory_percent']:.1f}%)")
                print(f"  GPU Load: {gpu_stats['gpu_load']:.1f}%")
        ...
        stats["performance/batch_time"] = batch_time
        
        # Log stats safely
        safe_log_stats(ppo_trainer, stats, batch, rewards)
        
        # Log performance metrics to WandB
        log_performance_metrics(wandb_run, epoch, batch_time, gpu_stats, system_stats)
    ...
    try:
        ...
        try:
            minimal_stats = {
                ...
                'performance/batch_time': clean_stats.get('performance/batch_time', 0.0)
            }
